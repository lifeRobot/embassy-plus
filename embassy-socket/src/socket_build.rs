...
pub trait SocketBuilderTrait<'d, const N: usize, const TX_SZ: usize, const RX_SZ: usize, const BUF_SIZE: usize> {
    /// build tcp client
    fn build_tcp_client<CB: TcpClientCallBack, const RC_SZ: usize, const WC_SZ: usize>(self, ip: Ipv4Addr, port: u16, cb: CB, state: &'d SocketState<N, TX_SZ, RX_SZ, BUF_SIZE>, socket_channel: &'d SocketChannel<'d, RC_SZ, WC_SZ>,
...
for Stack<'d> {
    ...
    fn build_tcp_client<CB: TcpClientCallBack, const RC_SZ: usize, const WC_SZ: usize>(
        ...
        socket_channel: &'d SocketChannel<'d, RC_SZ, WC_SZ>) -> (ReadRunner<'d, N, TX_SZ, RX_SZ, BUF_SIZE, RC_SZ, WC_SZ>, CallbackRunner<'d, RC_SZ, CB>) {
